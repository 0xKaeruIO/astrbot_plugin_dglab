...
@register("astrbot_plugin_dglab", "0XKaeruIO", "使用郊狼电似你们喵的插件。", "v0.0.1")
class DGLabPlugin(Star):
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        conn = user_manager.ensure_commander(user_id, user_name, self._get_ip(), reply)
        commander = conn["commander"]
        commander._help_prefix = self._get_prefix()

        logger.info("用户 %s 执行 DGLAB 命令: %s", user_id, body.split()[0] if body else "")
